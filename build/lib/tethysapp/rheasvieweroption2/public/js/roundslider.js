...
(function ($, window, undefined) {
    ...
    RoundSlider.prototype = {
        ...
        defaults: {
            ...
            circleShape: "full",
            handleShape: "round",
            capping : true, // specify if capping in max or min while sliding
        ...
        _update: function () {
            ...
            if (!this._multiRangeSlider) this._handle1 = this._handle2 = this._handleDefaults();
        ...
        _createLayers: function () {
            ...
            if (this._multiRangeSlider || this._rangeSlider || this._showRange) {
        ...
        _setValue: function () {
            if (this._multiRangeSlider) for (var i = 0; i< this._handle.length; i++) this._setHandleValue(i);
            else if (this._rangeSlider) {
        ...
        _setHandleValue: function (index) {
            this._active = index;
            var handle = (this._multiRangeSlider ? this._handle[index] : this["_handle" + index]);
        ...
        _appendHandle: function () {
            if (this._multiRangeSlider) this._createMultiHandles();
            else {
        ...
        _createMultiHandles: function (){
            var temp = [];
            for(var i=0; i< this._handle.length; i++){
              var handle = createElement("div.rs-handle rs-move"), o = this.options, hs;
              if ((hs = o.handleShape) != "round") handle.addClass("rs-handle-" + hs);
              if (this.options.handleclasses) handle.addClass(this.options.handleclasses[i]);
              handle.attr({ "index": i, "tabIndex": "0" });

              var id = this._dataElement()[0].id, id = id ? id + "_" : "";
              var label = id + "handle_" +i;
              handle.attr({ "role": "slider", "aria-label": label });     // WAI-ARIA support

              var bar = createElement("div.rs-bar rs-transition").css("z-index", "7").append(handle).rsRotate(this._start);
              bar.addClass(i == this._handle.length-1 ? "rs-second" : (i == 0 ? "rs-first" : ""));
              this.container.append(bar);
              this._refreshHandle();

              this.bar = bar;
              this._active = i;
              //if (i != 0 && i != this._handle.length-1) this._handle[i] = this._handleDefaults();
              this._bind(handle, "focus", this._handleFocus);
              this._bind(handle, "blur", this._handleBlur);
        ...
        _activeHandleBar: function (index) {
            ...
            var ind = (this._multiRangeSlider ? index : index - 1);
            return $(this.container.children("div.rs-bar")[ind]);
        },
        ...
        _getMultiHandleValueString: function(){
            var t = "";
            for (var i = 0; i < this._handle.length; i ++){
              t += (i==this._handle.length-1 ? this._handle[i].value : this._handle[i].value+",")
            }
            return t;
        },
        // internal methods
        _changeSliderValue: function (value, angle) {
            ...
            else if (this._multiRangeSlider){

                var size = this._handle.length

                var prev_angle = this._handle[(this._active+size-1)%size].angle;
                var this_angle = angle;//this._handle[this._active].angle;
                var next_angle = this._handle[(this._active+1)%size].angle;
                // shift all angles by bring prev angle to zero
                var diff = 360 - prev_angle;
                prev_angle = 0;
                this_angle = (this_angle+diff)%360;
                next_angle = (next_angle+diff)%360;
                // console.log(prev_angle,this_angle,next_angle);

                if (this_angle >= prev_angle && this_angle <= next_angle){
                  this._handle[this._active] = { angle: angle, value: value };
                  this.options.value = this._getMultiHandleValueString();
                  this.bar.rsRotate(lAngle);
                  this._updateARIA(value);

                  var dAngle = this._oriAngle(this._handle[size-1].angle) - this._oriAngle(this._handle[0].angle), o2 = "1", o3 = "0";
                  if (dAngle <= 180 && !(dAngle < 0 && dAngle > -180)) o2 = "0", o3 = "1";
                  this.block2.css("opacity", o2);
                  this.block3.css("opacity", o3);

                  if (this._active == 0 || this._active == size - 1){
                    (this._active == 0 ? this.block4 : this.block2).rsRotate(lAngle - 180);
                    (this._active == 0 ? this.block1 : this.block3).rsRotate(lAngle);
                  }
                }
            }
            else if ((!this.options.capping)||((this._active == 1 && oAngle <= this._oriAngle(this._handle2.angle)) ||
        ...
        _updateARIA: function (value) {
            ...
            if (this._multiRangeSlider){
              // console.log(this.bar.children());
            }else if (this._rangeSlider){//this.options.sliderType == "range") {
        ...
        _checkAngle: function (angle, isDrag) {
            ...
            var preAngle;
            if (this._multiRangeSlider) preAngle = this._handle[this._active].angle;
            else preAngle = this["_handle" + this._active].angle;
        ...
        _getTooltipValue: function (isNormal) {
            if (this._multiRangeSlider){
              var p = this._handle.map(function(item, index){ return item.value});
              var t = "";
              for (var i = 0; i< p.length; i++){
                if (isNormal) t += (i == p.length-1 ? p[i] : p[i]+'-');
                else t += (i == p.length-1 ? this._tooltipValue(p[i], i) : this._tooltipValue(p[i], i)+"-");
              }
              return t;
            }
            else if (this._rangeSlider) {
        ...
        _validateSliderType: function () {
            ...
            else if (type == "multirange") this._multiRangeSlider = this._showRange = true;
        ...
        _validateModelValue: function () {
            ...
            if (this._multiRangeSlider) {
                ...
                val = []
                for (var i =0; i < parts.length; i++){
                  val[i] = this._limitValue(parseFloat(parts[i]));
                }
                if (this._invertRange) val = val.reverse();
                this._handle = [];
                var t = "";
                for (var i = 0; i < parts.length; i++){
                  this._handle[i] = this._processStepByValue(val[i]);
                  t += this._handle[i].value;
                  t += (i == parts.length-1) ? "": ",";
                }
                this.options.value = t;
            }
                ...
                if (this.option.capping) if (!this._invertRange) if (val1 > val2) val2 = val1;
